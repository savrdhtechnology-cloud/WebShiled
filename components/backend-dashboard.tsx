...
function Notice({ message }: { message: string }) {
  return <article className="panel" style={{marginTop:16}}><div className="panelHead"><div><h3>Connected Traffic Status</h3><p>{message}</p></div><span className="statusTag medium">TRAFFIC ACCESS REQUIRED</span></div><p>External scan intelligence above is real. Visitor IPs, country traffic, bot requests and attack events require request/log access from the protected website.</p><div className="buttonRow"><Link href="/register" className="btn small">Create real account</Link><Link href="/app/live-visitors" className="ghostBtn small">Traffic Intelligence</Link></div></article>;
}
...
export function BackendDashboard({ admin = false }: { admin?: boolean }) {
  ...
  useEffect(() => {
    fetch(admin ? "/api/admin" : "/api/dashboard", { cache: "no-store" })
      .then(async r => {
        ...
        if (!r.ok || !body.ok) throw new Error(body?.error?.message || "Connected backend data unavailable.");
        return body.data;
      })
      .then(setData)
      .catch(e => setError(e instanceof Error ? e.message : "Connected backend data unavailable."))
  ...
  if (admin) {
    ...
    return <div className="pageWrap">
      ...
      {loading && <article className="panel" style={{marginTop:16}}><p>Loading connected backend data…</p></article>}
      {error && <Notice message={error}/>} 
      {data && <>
        <div className="metricGrid compact" style={{marginTop:16}}>{stats.map(([label,value,note,icon])=><article className="metricCard" key={label}><div className="metricTop"><span>{label}</span><div className="metricIcon"><Icon name={icon}/></div></div><strong>{value}</strong><small>{note}</small></article>)}</div>
  ...
  return <div className="pageWrap">
    <div className="pageHeader"><div><div className="breadcrumbs">WebShield <span>/</span> Overview</div><h1>Security Dashboard</h1><p>Scan-first security intelligence now; real visitor and threat telemetry after traffic integration.</p></div><span className="statusTag safe">PRE-INTEGRATION INTELLIGENCE</span></div>
    <WebsiteScanner />
    <PreIntegrationIntelligence compact/>

    {loading && <article className="panel" style={{marginTop:16}}><p>Loading connected traffic/backend data…</p></article>}
    {error && <Notice message={error}/>} 
    {m && <>
      ...
      <div className="dashboardGrid equal">
        <article className="panel"><div className="panelHead"><div><h3>Recent Threats</h3><p>Latest real threat_events records.</p></div><Link href="/app/threat-center" className="textButton">View all →</Link></div>{(data?.threats || []).length ? (data?.threats || []).map(t=><div className="eventRow" key={t.id}><span className={`alertIcon ${(t.severity || "low").toLowerCase()}`}><Icon name="threat"/></span><div><b>{t.type}</b><small>{t.source_ip || "Unknown IP"} · {t.target_url || "—"}</small></div><span className="riskScore">{t.risk_score}</span></div>) : <p>No connected threat events recorded yet.</p>}</article>
        <article className="panel"><div className="panelHead"><div><h3>Recent Alerts</h3><p>Latest real alerts records.</p></div><Link href="/app/alerts" className="textButton">View all →</Link></div>{(data?.alerts || []).length ? (data?.alerts || []).map(a=><div className="eventRow" key={a.id}><span className={`alertIcon ${(a.severity || "low").toLowerCase()}`}><Icon name="bell"/></span><div><b>{a.title}</b><small>{a.message || a.type}</small></div><span>{a.acknowledged_at ? "ACK" : "OPEN"}</span></div>) : <p>No connected alerts recorded yet.</p>}</article>
      </div>
    </>}
  </div>;
}
